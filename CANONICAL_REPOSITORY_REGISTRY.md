...
| AutoMarketer | `app-automarketer-891ef675` | **CANONICAL / RESOLVED / VERIFIED** | `app-automarketer` explicitly declares itself superseded and points to the maintained Universal AI Growth OS implementation. The canonical tree is protected against committed environment files; locked npm and Bun installs, lint, 7 tests and production build pass, and the permanent dependency audit passes production-moderate and full-tree-high thresholds after controlled Router/Vite/Vitest hardening. |
| ScrollJustice / ScrollCourt | `scroll-justice-prophecy-ai-40` | **CANONICAL / RESOLVED** | Public `scroll-justice-prophecy-ai` already declares itself superseded and points to the private developed successor. The canonical repo contains the current litigation-intelligence architecture, evidence integrity, human-reviewed AI, RBAC/RLS, audit-chain controls and deployment documentation; its current tree is now protected against committed environment files. |
| Adelaide finance | `adelaide-ledger-ai` | **CANONICAL / RESOLVED** | `AdelaideAI` contains only the earlier shell and `AdelaideAccounts` was empty; both now redirect to the maintained private ledger application. The canonical repo contains authenticated dashboard, invoice, transaction, reporting, contact, profile and support flows, and its Supabase frontend configuration has been moved out of source into environment variables. |
| AI Studio | `stanley-ai-studio` | **CANONICAL / RESOLVED / VERIFIED** | `AI_Studio` is the legacy placeholder and redirects to the maintained application. The canonical repo independently passes read-only CI plus the permanent dependency audit at production-moderate and full-tree-high thresholds after controlled dependency hardening. |
